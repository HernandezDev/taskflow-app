...
- [ ] **[UI-006] Bugfix: Sincronización de View Transitions:** Diagnosticar y resolver artefactos visuales o asincronía en el cambio de rutas. Validar la integración del enrutador (`preact-iso`) con `document.startViewTransition`, asegurando que la captura del nuevo estado espere a la mutación completa del DOM en Preact.
...
- [ ] **[UI-004] Reconstrucción Nativa de `Editable`:** Reemplazo de la dependencia de terceros `@zag-js/editable` por una implementación puramente nativa con `@preact/signals`. Requiere programar la gestión de accesibilidad (`aria-editables`) y eventos de teclado (`Enter`/`Escape`) para purgar la librería.
- [ ] **[UI-005] Reconstrucción Nativa de `RadioGroup`:** Eliminar la dependencia `@zag-js/radio-group` construyendo un control de selección de estados semántico desde cero, apalancándose en `<input type="radio">` ocultos, Tailwind CSS y signals para el estado.
